...
public class DashboardFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        View btnQuickAddOrder = view.findViewById(R.id.btnQuickAddOrder);
        View btnViewOrdersAction = view.findViewById(R.id.btnViewOrders);

        if (btnLogout != null) {
            btnLogout.setOnClickListener(v -> handleLogout());
        }

        if (btnQuickAddOrder != null) {
            btnQuickAddOrder.setOnClickListener(v -> {
        ...
        if (btnViewOrdersAction != null) {
            btnViewOrdersAction.setOnClickListener(v -> navigateToOrders());
        }

        setupIcons(view);
        setupWelcomeText(tvWelcome);
        updateDashboardStats();

        if (tvViewAll != null) {
            tvViewAll.setOnClickListener(v -> navigateToOrders());
        }
    ...
    private void setupWelcomeText(TextView tvWelcome) {
        FragmentActivity activity = getActivity();
        if (activity != null) {
            SharedPreferences prefs = activity.getSharedPreferences("LaundryPrefs", Context.MODE_PRIVATE);
            ...
            if (start >= 0) {
                ...
                spannableString.setSpan(new ForegroundColorSpan(0xFF2563EB), start, end, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
            }
    ...
    private TextView createStatusCell(String status) {
        ...
        int statusColor;
        if ("Ready for Pickup".equalsIgnoreCase(status)) statusColor = 0xFF7C3AED;
        else if ("Completed".equalsIgnoreCase(status)) statusColor = 0xFF16A34A;
        else if ("Washing".equalsIgnoreCase(status)) statusColor = 0xFF2563EB;
        else statusColor = 0xFFEA580C;
